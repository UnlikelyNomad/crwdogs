...
class RegistrationTableMap extends TableMap
{
     ...
     */
    const CLASS_NAME = 'crwdogs.events.Map.RegistrationTableMap';
     ...
     */
    const OM_CLASS = '\\crwdogs\\events\\Registration';

    /**
     * A class that can be returned by this tableMap
     */
    const CLASS_DEFAULT = 'crwdogs.events.Registration';
     ...
     */
    const COL_REGISTRATION_ID = 'registration.registration_id';

    /**
     * the column name for the event_id field
     */
    const COL_EVENT_ID = 'registration.event_id';

    /**
     * the column name for the user_id field
     */
    const COL_USER_ID = 'registration.user_id';
  ...
  array (
    ...
    1 => ':user_id',
        ...
        $this->addRelation('Payment', '\\crwdogs\\events\\Payment', RelationMap::ONE_TO_MANY, array (
  0 =>
  array (
    0 => ':registration_id',
    1 => ':registration_id',
  ...
  array (
    ...
    public static function getPrimaryKeyHashFromRow($row, $offset = 0, $indexType = TableMap::TYPE_NUM)
    {
        ...
        if ($row[TableMap::TYPE_NUM == $indexType ? 0 + $offset : static::translateFieldName('RegistrationId', TableMap::TYPE_PHPNAME, $indexType)] === null) {
            return null;
        }

        return null === $row[TableMap::TYPE_NUM == $indexType ? 0 + $offset : static::translateFieldName('RegistrationId', TableMap::TYPE_PHPNAME, $indexType)] || is_scalar($row[TableMap::TYPE_NUM == $indexType ? 0 + $offset : static::translateFieldName('RegistrationId', TableMap::TYPE_PHPNAME, $indexType)]) || is_callable([$row[TableMap::TYPE_NUM == $indexType ? 0 + $offset : static::translateFieldName('RegistrationId', TableMap::TYPE_PHPNAME, $indexType)], '__toString']) ? (string) $row[TableMap::TYPE_NUM == $indexType ? 0 + $offset : static::translateFieldName('RegistrationId', TableMap::TYPE_PHPNAME, $indexType)] : $row[TableMap::TYPE_NUM == $indexType ? 0 + $offset : static::translateFieldName('RegistrationId', TableMap::TYPE_PHPNAME, $indexType)];
    }
    ...
    public static function getPrimaryKeyFromRow($row, $offset = 0, $indexType = TableMap::TYPE_NUM)
    {
        return (int) $row[
            $indexType == TableMap::TYPE_NUM
                ? 0 + $offset
                : self::translateFieldName('RegistrationId', TableMap::TYPE_PHPNAME, $indexType)
        ];
    }
    ...
    {
        return $withPrefix ? RegistrationTableMap::CLASS_DEFAULT : RegistrationTableMap::OM_CLASS;
    }
    ...
    {
        $key = RegistrationTableMap::getPrimaryKeyHashFromRow($row, $offset, $indexType);
        if (null !== ($obj = RegistrationTableMap::getInstanceFromPool($key))) {
            // We no longer rehydrate the object, since this can cause data loss.
            // See http://www.propelorm.org/ticket/509
            // $obj->hydrate($row, $offset, true); // rehydrate
            $col = $offset + RegistrationTableMap::NUM_HYDRATE_COLUMNS;
        } else {
            $cls = RegistrationTableMap::OM_CLASS;
            /** @var Registration $obj */
            $obj = new $cls();
            $col = $obj->hydrate($row, $offset, false, $indexType);
            RegistrationTableMap::addInstanceToPool($obj, $key);
        }
    ...
    public static function populateObjects(DataFetcherInterface $dataFetcher)
    {
        ...
        while ($row = $dataFetcher->fetch()) {
            $key = RegistrationTableMap::getPrimaryKeyHashFromRow($row, 0, $dataFetcher->getIndexType());
            if (null !== ($obj = RegistrationTableMap::getInstanceFromPool($key))) {
                // We no longer rehydrate the object, since this can cause data loss.
                // See http://www.propelorm.org/ticket/509
    ...
    {
        if (null === $alias) {
            $criteria->addSelectColumn(RegistrationTableMap::COL_REGISTRATION_ID);
    ...
    {
        return Propel::getServiceContainer()->getDatabaseMap(RegistrationTableMap::DATABASE_NAME)->getTable(RegistrationTableMap::TABLE_NAME);
    }
    ...
    {
        $dbMap = Propel::getServiceContainer()->getDatabaseMap(RegistrationTableMap::DATABASE_NAME);
        if (!$dbMap->hasTable(RegistrationTableMap::TABLE_NAME)) {
            $dbMap->addTableObject(new RegistrationTableMap());
        }
    }

    /**
     * Performs a DELETE on the database, given a Registration or Criteria object OR a primary key value.
     *
     ...
     */
     public static function doDelete($values, ConnectionInterface $con = null)
     {
        ...
        if ($values instanceof Criteria) {
            // rename for clarity
            $criteria = $values;
        } elseif ($values instanceof \crwdogs\events\Registration) { // it's a model object
            // create criteria based on pk values
            $criteria = $values->buildPkeyCriteria();
        } else { // it's a primary key, or an array of pks
            $criteria = new Criteria(RegistrationTableMap::DATABASE_NAME);
            $criteria->add(RegistrationTableMap::COL_REGISTRATION_ID, (array) $values, Criteria::IN);
        }

        $query = RegistrationQuery::create()->mergeWith($criteria);

        if ($values instanceof Criteria) {
            RegistrationTableMap::clearInstancePool();
        } elseif (!is_object($values)) { // it's a primary key, or an array of pks
            foreach ((array) $values as $singleval) {
                RegistrationTableMap::removeInstanceFromPool($singleval);
            }
        }
    ...
    {
        return RegistrationQuery::create()->doDeleteAll($con);
    }

    /**
     * Performs an INSERT on the database, given a Registration or Criteria object.
     *
     * @param mixed               $criteria Criteria or Registration object containing data that is used to create the INSERT statement.
    ...
    public static function doInsert($criteria, ConnectionInterface $con = null)
    {
        ...
        if ($criteria instanceof Criteria) {
            ...
            $criteria = $criteria->buildCriteria(); // build Criteria from Registration object
        }

        if ($criteria->containsKey(RegistrationTableMap::COL_REGISTRATION_ID) && $criteria->keyContainsValue(RegistrationTableMap::COL_REGISTRATION_ID) ) {
...
//
RegistrationTableMap::buildTableMap();
